...
```{r}
Prob.Ar <- (factorial(4) / factorial(2)) * (5 / 8)^2 * (3/8)^2
Prob.Ar
Prob.Aw <- ((factorial(5) / factorial(2)) * (factorial(3) / factorial(2))) / (factorial(8) / factorial(4))
Prob.Aw
```
...
Draw_Ball <- function(){
  ...
  bag <- rep(ball, c(3, 5))
  result <- sample(bag, 4)
  table(factor(result, levels=ball))
}
...
```{r}
library(readxl)
award.list <- read_excel("C:/Users/蕭亞青/Documents/SHU-R/data/award-list.xlsx")
```
...
```{r}
str(award.list)
substr(award.list$會員姓名, 2, 2) <- ("*")
award.list$會員卡號 <- as.character(award.list$會員卡號)
substr(award.list$會員卡號, 5, 7) <- ("***")
data.frame(award.list)
```
...
```{r}
award.list$得獎金額 <- sort(award.list$得獎金額)
data.frame(award.list)
```
...
```{r}
IQV <- function(k, n){
  (k * ((n^2) - sum(k^2))) / (n^2 * (k - 1))
}
set.seed(12345)
no <- sample(20:100, 1)
nv <- LETTERS[sample(1:26, 5)][sample(1:5, no, replace=T)]
x <- as.numeric(table(nv))
IQV(x, no)
```
